...
- Entry with `tmux_session` set: `tmux has-session -t <tmux_session>`.
  ...
  If *many* entries fail lookup at once, the tmux server died — go to
  § Crash recovery and read the evidence in the order it gives before
  changing any state or recreating anything. Surface aggregately.
  Unlike numeric window ids, `tmux_session` is a user-namespace name
  and cannot be recycled into pointing at something unrelated — the
...
7. Update the registry under lock:
   ...
   - Remove `shutdown`, `snapshot` and `resume_state`.
   - **Keep `resumed_session_id`**, and add a `worker:` line for every
     other Claude pane the rebuild brought back.
   - Update `last_touched`.

   **Leave the on-disk snapshot and resume_state files alone.** Deleting
   them on the grounds that the live session supersedes them makes a
   resumed session *less* recoverable than a shutdown one: it holds no
   recovery state again until its next clean shutdown, which is not
   guaranteed to come. They cost nothing to keep and the next shutdown
   overwrites them. If a stale one is confusing, note the supersession
   in the file rather than removing it.
